...
def test_assertions(page):
    ...
    expect(api_response).not_to_be_ok()  # status code is not in `200..299` range
    ...
    menu_items = ["Dashboard", "Rating", "Test Cases", "Test Runs", "Create new test", "Demo pages", "Demo controls"]
    expect(page.locator("div.menuBox").get_by_role("listitem")).to_have_count(7, timeout=1000)
